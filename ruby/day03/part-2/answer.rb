...
class Wire
  ...
  attr_reader :array_of_coords, :coords, :hash
  ...
  def affect_y_coords(value)
    if value > 0
      value.times do
        ...
        @steps +=1
        @hash["#{current_pos.first},#{current_pos.last}"] = @steps unless @hash["#{current_pos.first},#{current_pos.last}"]
        @coords << current_pos
      end
    else
      value.abs.times do
...
hash_1 = Wire.new(wires.first).hash
hash_2 = Wire.new(wires.last).hash
...
min = nil
min_pair = nil
crosses.each do |pair|
  if min == nil
    min = hash_1["#{pair.first},#{pair.last}"] + hash_2["#{pair.first},#{pair.last}"]
    min_pair = pair
  else
    if hash_1["#{pair.first},#{pair.last}"] + hash_2["#{pair.first},#{pair.last}"] < min
      min = hash_1["#{pair.first},#{pair.last}"] + hash_2["#{pair.first},#{pair.last}"]
      min_pair = pair
    end
  end
...
# p crosses.first.first.abs + crosses.first.last.abs
